...
class mcworld(object):
    def __init__(self, leveldir, namespace):
        ...
        self.zip = zipfile.ZipFile(
            self.zipbytes, "w", zipfile.ZIP_DEFLATED, False
        )
    ...
    def write_tags(self, clocks, block_tags, entity_tags, item_tags):
        ...
        self.zip.writestr(
            tick_tag_file,
            json.dumps(
                {"values": [f"{self.namespace}:{name}" for name in clocks]},
                indent=4,
            ),
        )
        ...
        self.zip.writestr(
            load_tag_file,
            json.dumps({"values": [f"{self.namespace}:reset"]}, indent=4),
        )
                ...
                for tag in list:
                    ...
                    self.zip.writestr(
                        ...
                        json.dumps(
                            {
                                "values": [
                                    f"minecraft:{item}" for item in items
                                ]
                            },
                            indent=4,
                        ),
        ...
        if len(recipes) > 0:
            ...
            for recipe in recipes:
                ...
                recipe_file = os.path.join(
                    recipe_dir, f"{recipe.get_type()}{id}.json"
                )
    ...
    def write_advancements(self, advancements):
        if len(advancements) > 0:
            ...
            for name in advancements:
                advancement_file = os.path.join(
                    advancement_dir, f"{name}.json"
                )
        ...
        if len(loot_tables) > 0:
            for name in loot_tables:
                ...
                if ":" in name:
                    ...
                    if len(parts) != 2:
                        raise CompileError(
                            f'Invalid loot tables name "{name}"'
                        )
                    loot_table_dir = f"data/{parts[0]}/loot_tables/{type}/"
                    filename = parts[1]
                else:
                    loot_table_dir = (
                        f"data/{self.namespace}/loot_tables/{type}/"
                    )
                    filename = name
                loot_table_file = os.path.join(
                    loot_table_dir, f"{filename}.json"
                )
    ...
    def write_item_modifiers(self, item_modifiers):
        if len(item_modifiers) > 0:
            ...
            for name in item_modifiers:
                item_modifier_file = os.path.join(
                    item_modifier_dir, f"{name}.json"
                )
    ...
    def write_zip(self):
        ...
        zip_filename = os.path.join(
            self.dir, f"datapacks/{self.namespace}.zip"
        )
