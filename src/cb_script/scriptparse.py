...
def p_file_param(p):
    ...
    if p[1] not in ["scale"]:
        raise SyntaxError(
            f'Unknown file parameter: "{p[1]}" at line {p.lineno(1)}'
        )
...
def p_execute_on(p):
    ...
    if p[2] not in [
        ...
        print(
            f'Warning: Unknown argument for execute on: "{p[2]}" at line {p.lineno(2)}'
        )
...
def p_execute_align(p):
    ...
    if p[2] not in ["x", "y", "z", "xy", "xz", "yz", "xyz"]:
        raise SyntaxError(
            f'Must align to a combination of x, y, and z axes, not "{p[2]}", at line {p.lineno(2)}'
        )
    p[0] = ("Align", p[2])
...
def p_uuid(p):
    ...
    p[0] = (
        f"{int(p[1]):X}-{int(p[3]):X}-{int(p[5]):X}-{int(p[7]):X}-{int(p[9]):X}"
    )
...
def p_data_type(p):
    ...
    if p[1] not in ["byte", "double", "float", "int", "long", "short"]:
        raise SyntaxError(
            f'Syntax Error: Invalid path type "{{}}" at line {p.lineno(1)}.'
        )
    p[0] = p[1]
...
def p_array_definition(p):
    ...
    p[0] = array_definition_block(
        p.lineno(1), p[2], const_number(0), p[4], False
    )
...
def p_return_expression(p):
    ...
    p[0] = scoreboard_assignment_block(
        p.lineno(1), scoreboard_var("Global", "ReturnValue"), "=", p[2]
    )
...
def p_assignment_create(p):
    ...
    p[0] = scoreboard_assignment_block(
        p.lineno(1), p[1], p[2], create_expr(p[3])
    )
...
def p_json_value_typed_number(p):
    ...
    if p[2].lower() not in ["b", "f", "s", "d", "l"]:
        raise SyntaxError(
            f'Invalid type "{p[2]}" for number "{p[1]}" at line {p.lineno(1)}'
        )
...
def p_json_literal_array(p):
    ...
    if p[3].lower() not in ["b", "i", "l"]:
        raise SyntaxError(
            f'Invalid type "{p[3]}" for literal array at line {p.lineno(1)}'
        )
...
def p_recipe_key_item(p):
    ...
    if len(p[1]) != 1:
        raise SyntaxError(
            f'Recipe key "{p[1]}" at line {p.lineno(1)} is not a single character.'
        )

    if p[3] not in ["item", "tag"]:
        raise SyntaxError(
            f'Recipe key type "{p[3]}" at line {p.lineno(1)} must be "block" or "tag".'
        )
...
def p_loot_table_type(p):
    ...
    if p[1] not in types:
        raise SyntaxError(
            f'Invalid loot table type "{p[1]}" at line {p.lineno(1)}.'
        )
...
def p_loot_table_definition(p):
    ...
    p[0] = loot_table_definition_block(
        p.lineno(1), p[2], p[3] + p[4] + p[5], p[6]
    )
...
def p_error(p):
    if p == None:
        ...
        raise SyntaxError(
            'Syntax error at line {} column {}. Unexpected {} symbol "{}" in state {}.'.format(
                p.lineno,
                scriptlex.find_column(bparser.data, p),
                p.type,
                p.value.replace("\n", "\\n"),
                bparser.state,
            )
