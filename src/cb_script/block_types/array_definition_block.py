...
class array_definition_block(block_base):
    ...
    def compile(self, func):
        try:
            ...
            raise CompileError(
                f'Unable to get array range for "{self.name}" at line {self.line}'
            )
        ...
        cases = [
            (
                ...
                [
                    command_block(
                        self.line,
                        f"/scoreboard players operation {selector} {valvar} = {selector} {name}{i}",
                    )
                ],
                self.line,
                None,
        ...
        if not get_func.switch_cases(
            indexvar, cases, "arrayget", "arraygetidx"
        ):
            raise Exception(
                f"Error creating getter for array at line {self.line}"
            )
        ...
        if not set_func.switch_cases(
            indexvar, cases, "arrayset", "arraysetidx"
        ):
            raise Exception(
                f"Error creating setter for array at line {self.line}"
            )
