...
class for_index_block(block_base):
    ...
    def compile(self, func):
        ...
        if by == None:
            ...
            loop_func.add_command(
                f"scoreboard players add {temp_var.get_selvar(func)} 1"
            )
            ...
            if by_const:
                ...
                loop_func.add_command(
                    f'scoreboard players {"add" if by_const > 0 else "remove"} {temp_var.get_selvar(func)} {abs(by_const)}'
                )
            else:
                ...
                loop_func.add_command(
                    f"scoreboard players operation {temp_var.get_selvar(func)} += {by_var.get_selvar(func)}"
                )
