...
class selector_definition:
    def __init__(self, selector, env):
        ...
        if base_name in env.selectors:
            ...
            if len(base_name) != 1:
                raise CompileError(
                    f"Tried to create selector with base name @{base_name}"
                )
        ...
        if "[" in selector and "]" in selector:
            parts = selector[
                selector.find("[") + 1 : selector.rfind("]")
            ].split(",")
            ...
            if len(parts) == 1:
                try:
                    index = int(parts[0])
                    self.uuid = (
                        f"0-0-0-0-{index + hash(base_name) % (2 ** 32):X}"
                    )
                    return
                except:
                    None
            ...
            for part in parts:
                ...
                if op_parts == None:
                    if "=" in part:
                        ...
                        if (
                            subparts[0] == "type"
                            and not type.startswith("minecraft:")
                            and not type.startswith("!minecraft:")
                        ):
                            if subparts[1].startswith("!"):
                                if type in env.entity_tags:
                                    subparts[1] = (
                                        f"!#{env.namespace}:{subparts[1][1:]}"
                                    )
                                else:
                                    subparts[1] = (
                                        "!minecraft:" + subparts[1][1:]
                                    )
                            else:
                                if type in env.entity_tags:
                                    subparts[1] = (
                                        f"#{env.namespace}:{subparts[1]}"
                                    )
                                else:
                    ...
                    if not isNumber(after):
                        raise SyntaxError(
                            f'"{after}" is not a number in "{selector}"'
                        )
            ...
            for var in self.scores_min:
                if var in self.scores_max:
                    if self.scores_min[var] == self.scores_max[var]:
                        ...
                        score_parts.append(
                            f"{var}={self.scores_min[var]}..{self.scores_max[var]}"
                        )
                else:
                    score_parts.append(f"{var}={self.scores_min[var]}..")
