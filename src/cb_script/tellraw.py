    ...
    for segment, properties in parseTextFormatting(text):
        ...
        if isinstance(segment, str):
            ...
            if unformatted == None:
                ...
                if len(parts) > 2:
                    raise CompileError(
                        f'Json text has invalid () text: "{command}"'
                    )
                if len(parts) == 1:
                    if len(parts[0]) == 0:
                        ...
                        if len(storage_parts) != 2:
                            raise CompileError(
                                f'Unable to parse storage variable "{parts[0]}"'
                            )
                        ...
                        formatted = (
                            formatted
                            + f',{{"nbt":"{storage_parts[1]}","storage":"{target}"}}'
                        )
                    else:
                        formatted = (
                            formatted
                ...
                if command.startswith("http"):
                    ...
                    action = "run_command"
                    command = (
                        f'/function {func.namespace}:{command[len("call "):]}'
                    )
                else:
                    action = "run_command"
                formatted = (
    ...
    for ch in text:
        if ch == "\\" and not escaped:
            ...
            if ch in COLORS:
                ...
                raise CompileError(
                    f"Unexpected formatting character {{{ch} in tell command"
                )
