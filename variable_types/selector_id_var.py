...
class selector_id_var(var_base):
    ...
    def initialize_id(self, func):
        if not func.check_single_entity(self.selector):
            raise CompileError(
                f'Selector "{self.selector}" does not specify an individual entity.'
            )
        ...
        func.add_command(
            f"execute unless score {self.selector} _id matches 1.. run scoreboard players operation {0} _id = Global _unique"
        )
