...
class template_function_call_block(call_block_base):
    ...
    def compile(self, func):
        ...
        if function not in func.template_functions:
            raise ValueError(
                f'Tried to call non-existant template function "{function}" at line {self.line}'
            )
        ...
        if len(template_args) != len(template_params):
            raise ValueError(
                f'Tried to call template function "{function}" with {len(template_args)} template arguments ({len(template_params)} expected) at line {self.line}'
            )

        if len(args) != len(params):
            raise ValueError(
                f'Tried to call template function "{function}" with {len(args)} function arguments ({len(params)} expected) at line {self.line}'
            )
        ...
        if func_name not in func.functions:
            ...
            for p in range(len(template_args)):
                new_env.set_dollarid(
                    template_params[p], template_args[p].get_value(func)
                )
            ...
            try:
                ...
                print(e.message)
                raise Exception(
                    f"Unable to compile template function contents at line {self.line}"
                )
