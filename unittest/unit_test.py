...
class test_cbscript(unittest.TestCase):
    ...
    def test_compile_runs_without_error(self):
        ...
        block = array_definition_block(
            0, "test", const_number("0"), const_number("5")
        )
        block.compile(func)
        ...
        block = for_index_block(
            0,
            "test",
            num_expr(0),
            num_expr(5),
            num_expr(2),
            [command_block(0, "test_command")],
        )
        block.compile(func)

        block = for_selector_block(
            0, "@test", "@a", [command_block(0, "test_command")]
        )
        block.compile(func)
        ...
        block = python_assignment_block(
            0, "test_id", interpreted_python("1+1", 0)
        )
        block.compile(func)

        block = python_for_block(
            0,
            "test_id",
            interpreted_python("range(3)", 0),
            [command_block(0, "test_command")],
        )
        block.compile(func)

        block = python_if_block(
            0,
            interpreted_python("1", 0),
            [command_block(0, "true_command")],
            [command_block(0, "false_command")],
        )
        block.compile(func)
        block = python_if_block(
            0,
            interpreted_python("0", 0),
            [command_block(0, "true_command")],
            [command_block(0, "false_command")],
        )
        block.compile(func)
        ...
        vpath = (
            "VectorPath",
            ("test_vector_id", "test_vector", "float", const_number(1)),
        )
        block = selector_definition_block(
            0, "test_id", "@a", [tag, path, vpath]
        )
        block.compile(func)
        ...
        case1 = (
            ...
            (
                "test_id",
                interpreted_python("range(3, 6)"),
                [command_block(0, "test_python")],
            ),
        )
        case2 = (
            "range",
            (
                const_number("1"),
                const_number("2"),
                [command_block(0, "test_range_1_2")],
            ),
        )
        ...
        func.template_functions["test_template_function"] = (
            [],
            [],
            [command_block(0, "test_command")],
        )
        block = template_function_call_block(
            0, "test_template_function", [], []
        )
        block.compile(func)
        ...
        block = title_block(
            ...
            "subtitle",
            "@a",
            [const_number("1"), const_number("2"), const_number("3")],
            "{rtest",
        )
        block.compile(func)

        var = ("VAR_ID", "test")
        block = vector_assignment_block(
            0, var, "+=", vector_here_expr(const_number(1000))
        )
        block.compile(func)
    ...
    def test_compile_vector_assignment_scalar(self):
        ...
        block = vector_assignment_scalar_block(
            0, ("VAR_ID", "test_vector"), "*=", num_expr(2)
        )
        block.compile(func)

        self.assertEqual(len(func.commands), 4)
        self.assertEqual(
            func.commands[0], "scoreboard players set Global test_scratch1 2"
        )
        for i in range(3):
            self.assertEqual(
                func.commands[i + 1],
                f"scoreboard players operation Global _test_vector_{i} *= Global test_scratch1",
            )
    ...
    def test_compile_vector_assignment(self):
        ...
        block = vector_assignment_block(
            ...
            ("VAR_ID", "test_vector"),
            "+=",
            vector_expr([num_expr(2), num_expr(3), num_expr(4)]),
        )
        block.compile(func)
        ...
        for i in range(3):
            self.assertEqual(
                func.commands[i],
                f"scoreboard players set Global test_scratch{i+1} {i+2}",
            )
            self.assertEqual(
                func.commands[i + 3],
                f"scoreboard players operation Global _test_vector_{i} += Global test_scratch{i+1}",
            )
    ...
    def test_compile_move(self):
        ...
        block.compile(func)
        self.assertTrue(
            "execute as @a at @s run tp @s ^0 ^0 ^1" in func.commands
        )
    ...
    def test_compile_python_assignment(self):
        ...
        block = python_assignment_block(
            0, "test", interpreted_python("1+1", 0)
        )
        block.compile(func)
        ...
        block = python_assignment_block(
            0, "test2", interpreted_python("1/0", 0)
        )
        ...
        block = python_assignment_block(
            0, "test3", interpreted_python("math.sqrt(9)", 0)
        )
        block.compile(func)
    ...
    def test_compile_array_assignment(self):
        ...
        block = array_assignment_block(
            0, "test_array", "Const", 1, num_expr(2)
        )
        block.compile(func)
        self.assertTrue(
            "scoreboard players set Global test_array1 2" in func.commands
        )

        func.arrays["test_array2"] = (0, 10)
        block = array_assignment_block(
            0, "test_array2", "Expr", num_expr(2), num_expr(3)
        )
        block.compile(func)
        self.assertTrue(
            "scoreboard players set Global test_array2Val 3" in func.commands
        )
        self.assertTrue(
            "scoreboard players set Global test_array2Idx 2" in func.commands
        )
        self.assertTrue(
            "function test_namespace:array_test_array2_set" in func.commands
        )
    ...
    def test_compile_array_definition(self):
        ...
        block = array_definition_block(
            0, "test_array", const_number(0), const_number(5)
        )
        block.compile(func)
    ...
    def test_compile_block_tag(self):
        ...
        block = block_tag_block(
            0, "test_block_tag", ["test_block", "test_block2"]
        )
        block.compile(func)

        self.assertTrue("test_block_tag" in func.block_tags)
        self.assertTrue(
            func.block_tags["test_block_tag"] == ["test_block", "test_block2"]
        )
    ...
    def test_compile_for_selector(self):
        ...
        block = for_selector_block(
            0, "test", "@a", command_block(1, "test_command")
        )
        block.compile(func)
    ...
    def test_compile_for_index(self):
        ...
        block = for_index_block(
            0, "test_idx", num_expr(1), num_expr(5), num_expr(2), []
        )
        block.compile(func)
    ...
    def test_compile_function_call(self):
        ...
        self.assertEqual(
            func.commands, ["function test_namespace:test_function"]
        )
    ...
    def test_compile_macro_call(self):
        ...
        self.assertEqual(
            func.cloned_environments[0].dollarid["test_param"], 10
        )
    ...
    def test_compile_method_call(self):
        ...
        block = method_call_block(
            0, "@test_selector", "test_method", [num_expr(1)]
        )
        block.compile(func)
    ...
    def test_compile_scoreboard_assignment(self):
        ...
        self.assertEqual(
            func.commands, ["/scoreboard players add Global var1 1"]
        )
        ...
        self.assertEqual(
            func.commands, ["/scoreboard players set Global var1 1"]
        )
        ...
        self.assertEqual(
            func.commands,
            [
                "/scoreboard players operation Global var1 *= test_constant Constant"
            ],
        )
    ...
    def test_compile_selector_definition(self):
        ...
        items = [
            ...
            (
                "Path",
                ("test_path", "path.to.data", "float", const_number(100)),
            ),
            (
                "VectorPath",
                (
        ...
        self.assertEqual(
            selector.paths["test_path"], ("path.to.data", "float", 100)
        )

        self.assertTrue("test_vector_path" in selector.vector_paths)
        self.assertEqual(
            selector.vector_paths["test_vector_path"],
            ("path.to.vector", "int", 200),
        )
    ...
    def test_compile_switch(self):
        ...
        self.assertEqual(
            func.commands, ["scoreboard players set Global test_scratch1 2"]
        )
    ...
    def test_compile_tell(self):
        ...
        self.assertEqual(
            func.commands,
            ['/tellraw @a ["",{"text":"hi","color":"dark_red"}]'],
        )
    ...
    def test_compile_template_function_call(self):
        ...
        block = template_function_call_block(
            0, "test_template_function", [const_number("10")], [num_expr("15")]
        )
        block.compile(func)
        ...
        self.assertTrue(
            "test_macro_param"
            in func.functions["test_template_function_10"].environment.dollarid
        )
        self.assertEqual(
            func.functions["test_template_function_10"].environment.dollarid[
                "test_macro_param"
            ],
            10,
        )
        ...
        block = template_function_call_block(
            0, "test_template_function", [python_identifier("test_id")], []
        )
        block.compile(func)

        self.assertTrue("test_template_function_5" in func.functions)
        self.assertEqual(
            func.functions["test_template_function_5"].environment.dollarid[
                "test_macro_param"
            ],
            5,
        )
    ...
    def test_compile_title(self):
        ...
        block = title_block(
            ...
            "subtitle",
            "@a",
            [const_number("0"), const_number("1"), const_number("2")],
            "{rtest",
        )
        block.compile(func)
    ...
    def test_compile_while(self):
        ...
        self.assertEqual(
            func.commands,
            ["execute_dummy run function test_namespace:while001_ln0"],
        )
        self.assertEqual(func.execute_command_calls, ["test"])
        self.assertEqual(len(func.child_functions), 1)
        self.assertEqual(
            func.child_functions[0].compiled_blocks[0], ["test_block"]
        )
        self.assertTrue(
            "execute_dummy run function test_namespace:while001_ln0"
            in func.child_functions[0].commands
        )
    ...
    def test_binop_expr(self):
        ...
        self.assertTrue(
            "scoreboard players set Global test_id 3" in func.commands
        )
        self.assertTrue(
            "scoreboard players add Global test_id 4" in func.commands
        )
        ...
        self.assertTrue(
            "scoreboard players add Global test_id2 3" in func.commands
        )
    ...
    def test_func_expr(self):
        ...
        expr = func_expr(
            function_call_block(0, "rand", [num_expr(2), num_expr(10)])
        )
        id = expr.compile(func, "test_id")
        ...
        self.assertTrue(
            "function test_namespace:test_function" in func.commands
        )
    ...
    def test_num_expr(self):
        ...
        self.assertEqual(id, "test_id")
        self.assertTrue(
            "scoreboard players set Global test_id 5" in func.commands
        )
    ...
    def test_scale_expr(self):
        ...
        self.assertEqual(id, "test_id")
        self.assertTrue(
            "scoreboard players set Global test_id 1000" in func.commands
        )
    ...
    def test_selvar_expr(self):
        ...
        self.assertEqual(id, "test_id")
        self.assertTrue(
            "scoreboard players operation Global test_id = @a test_var"
            in func.commands
        )
    ...
    def test_unary_expr(self):
        ...
        self.assertTrue(
            "scoreboard players operation Global test_id *= minus Constant"
            in func.commands
        )
    ...
    def test_sel_vector_var_expr(self):
        ...
        for left, right in [("x", 0), ("y", 1), ("z", 2)]:
            self.assertTrue(
                f"scoreboard players operation Global {left} = @test _test_var_{right}"
                in func.commands
            )
    ...
    def test_vector_binop_scalar_expr(self):
        ...
        expr = vector_binop_scalar_expr(
            vector_var_expr("test_vector"), "*", num_expr(2)
        )
        ids = expr.compile(func, ["x", "y", "z"])
    ...
    def test_vector_binop_vector_expr(self):
        ...
        expr = vector_binop_vector_expr(
            vector_var_expr("test_vector1"),
            "+",
            vector_var_expr("test_vector2"),
        )
        ids = expr.compile(func, ["x", "y", "z"])
    ...
    def test_vector_expr(self):
        ...
        for left, right in [("x", 1), ("y", 2), ("z", 3)]:
            self.assertTrue(
                f"scoreboard players set Global {left} {right}"
                in func.commands
            )
    ...
    def test_vector_var_expr(self):
        ...
        self.assertEqual(
            ids, ["_test_vector_0", "_test_vector_1", "_test_vector_2"]
        )
    ...
    def test_selector_definition(self):
        ...
        env = mock_environment()
        env.selectors["test_selector"] = selector_definition(
            '@a[nbt={test:"test_val"}]', env
        )

        sel = selector_definition(
            '@test_selector[nbt={test2:"test_val2"}]', env
        )
        # TODO: merge nbt tags via the json module
        self.assertEqual(
            sel.compile(), '@a[nbt={test:"test_val"},nbt={test2:"test_val2"}]'
        )
        ...
        self.assertEqual(
            sel.compile(), "@e[type=creeper,scores={test=0,test2=1..}]"
        )
        ...
        sel = selector_definition(
            '@e[nbt={test:"test_val",test2:{test3:"test_val3"}}]', env
        )
        # TODO: merge nbt tags via the json module
        self.assertEqual(
            sel.compile(),
            '@e[nbt={test:"test_val",test2:{test3:"test_val3"}}]',
        )
    ...
    def test_get_friendly_name(self):
        self.assertEqual(
            global_context.get_friendly_name("test .,:{}="), "CBtest_______"
        )
    ...
    def test_scratch_tracker_vector(self):
        ...
        self.assertEqual(
            st.get_scratch_vector(),
            ["test_scratch0", "test_scratch1", "test_scratch2"],
        )
    ...
    def test_cbscript_check_for_update(self):
        ...
        script.try_to_compile = new.instancemethod(
            count_compiles, script, None
        )
    ...
    def test_cbscript_try_to_compile(self):
        ...
        self.assertEqual(
            script.log_lines,
            ["Compiling unittest...", "Script successfully applied."],
        )
        ...
        self.assertEqual(
            script.log_lines,
            ["Compiling unittest...", "Script had compile error(s).\x07"],
        )
        ...
        script.try_to_compile()
        self.assertEqual(
            script.log_lines, ["Compiling unittest...", "Test error\x07"]
        )
        ...
        script.log_traceback = new.instancemethod(
            mock_log_traceback, script, None
        )
    ...
    def test_cbscript_compile_all(self):
        ...
        parsed["scale"] = 100
        parsed["assignments"] = [
            block_tag_block(0, "test_tag", ["test_block"])
        ]
        parsed["sections"] = [
            ("macro", "test_macro", [], [], []),
                ...
                [
                    scoreboard_assignment_block(
                        ...
                        (
                            ...
                            "=",
                            func_expr(
                                function_call_block(0, "rand", [num_expr(5)])
                            ),
                        ),
                    )
                ],
        ...
        script.create_world = new.instancemethod(
            mock_create_world, script, None
        )
        ...
        self.assertEqual(
            script.global_context.get_reset_function().commands,
            [
    ...
    def test_tellraw_get_properties_text(self):
        ...
        self.assertEqual(
            tellraw.getPropertiesText(properties),
            ',"color":"red","bold":true,"underlined":true,"italic":true,"strikethrough":true',
        )
    ...
    def test_tellraw_format_json_text(self):
        ...
        self.assertEqual(
            tellraw.formatJsonText(func, "test"), '["",{"text":"test"}]'
        )
    ...
    def test_parser(self):
        ...
        p = mock_parsed(
            ["comment"],
            ("type", "name", ["template_params"], ["params"], ["lines"]),
        )
        ...
        p = mock_parsed(
            None, "id", None, ["id_list"], None, None, "lines", None
        )
        ...
        p = mock_parsed(
            None, "id", None, "from", None, "to", None, "lines", None
        )
        ...
        p = mock_parsed(
            relcoord("", const_number("1")),
            relcoord("", const_number("2")),
            relcoord("", const_number("3")),
        )
        func = mock_mcfunction()
        ...
        self.assertEqual(
            p[0], (("Var", ("Global", "ReturnValue")), "=", "expr")
        )
        self.assertEqual(mcfunction.get_line(p[0]), 0)
        ...
        p = mock_parsed(
            "id", None, "macro_params", None, None, "function_params", None
        )
    ...
    def test_mcfunction_get_modifiable_id(self):
        ...
        self.assertEqual(
            func.commands,
            ["scoreboard players operation Global assign = Global test"],
        )
        ...
        self.assertEqual(
            func.commands,
            [
                "scoreboard players operation Global test_scratch1 = Global test"
            ],
        )
    ...
    def test_mcfunction_get_variable_path(self):
        ...
        self.assertEqual(
            func.commands,
            [
                "execute store result score @test_selector test_path run data get entity @test_selector path 100"
            ],
        )
    ...
    def test_mcfunction_set_variable_var(self):
        ...
        self.assertEqual(
            func.commands,
            [
                "execute store result entity @test_selector path float 0.01 run scoreboard players get @test_selector test_path"
            ],
        )
    ...
    def test_mcfunction_set_variable_arrayexpr(self):
        ...
        func.set_variable(var)
        self.assertEqual(
            func.commands, ["function test_namespace:array_test_array_set"]
        )
    ...
    def test_mcfunction_get_if_chain_score_num(self):
        ...
        for op, match in [
            ...
            condition = "score", (
                ("Var", ("@test_selector", "test_var")),
                op,
                ("num", interpreted_python("1")),
            )
            chain = func.get_if_chain([condition])

            self.assertEqual(
                chain, f"if score @test_selector test_var matches {match} "
            )
    ...
    def test_mcfunction_get_if_chain_score_var(self):
        ...
        condition = "score", (
            ("Var", ("@test_selector", "test_var")),
            ">",
            ("score", ("Var", ("@test_selector2", "test_var2"))),
        )
        chain = func.get_if_chain([condition])

        self.assertEqual(
            chain,
            "if score @test_selector test_var > @test_selector2 test_var2 ",
        )
    ...
    def test_mcfunction_get_if_chain_pointer(self):
        ...
        self.assertEqual(
            chain, "if score @test_selector test_var = @test_selector2 _id "
        )
    ...
    def test_mcfunction_get_if_chain_vector_equality_var_id(self):
        ...
        self.assertEqual(
            chain,
            "if score Global _var1_0 = Global _var2_0 if score Global _var1_1 = Global _var2_1 if score Global _var1_2 = Global _var2_2 ",
        )
    ...
    def test_mcfunction_get_if_chain_vector_equality_sel_var_id(self):
        ...
        condition = "vector_equality", (
            ("SEL_VAR_ID", ("@sel1", "var1")),
            ("SEL_VAR_ID", ("@sel2", "var2")),
        )
        chain = func.get_if_chain([condition])

        self.assertEqual(
            chain,
            "if score @sel1 _var1_0 = @sel2 _var2_0 if score @sel1 _var1_1 = @sel2 _var2_1 if score @sel1 _var1_2 = @sel2 _var2_2 ",
        )

    def test_mcfunction_mcfunction_get_if_chain_vector_equality_var_components(
        self,
    ):
        env = mock_environment()
        func = mcfunction.mcfunction(env)
        ...
        condition = "vector_equality", (
            ("VAR_COMPONENTS", [("a1", "x1"), ("b1", "y1"), ("c1", "z1")]),
            ("VAR_COMPONENTS", [("a2", "x2"), ("b2", "y2"), ("c2", "z2")]),
        )
        chain = func.get_if_chain([condition])

        self.assertEqual(
            chain,
            "if score a1 x1 = a2 x2 if score b1 y1 = b2 y2 if score c1 z1 = c2 z2 ",
        )
    ...
    def test_mcfunction_mcfunction_get_if_chain_block(self):
        ...
        self.assertEqual(
            chain,
            "if block 1 2 3 #test_namespace:test_block_tag if block 4 5 6 minecraft:test_block2 ",
        )
    ...
    def test_mcfunction_execute_command_as_id_attype(self):
        ...
        self.assertEqual(
            func.commands,
            ["scoreboard players operation Global _id = Global test_var"],
        )
    ...
    def test_mcfunction_switch_cases_four(self):
        ...
        self.assertTrue(
            func.switch_cases("switch_var", [case1, case2, case3, case4])
        )
        self.assertEqual(
            func.commands,
            [
    ...
    def test_mcfunction_switch_cases_three(self):
        ...
        self.assertEqual(
            env.cloned_environments[1].dollarid["test_python_var"], 3
        )
    ...
    def test_mcfunction_switch_cases_multiline(self):
        ...
        case2 = (
            3,
            3,
            [mock_block("command2"), mock_block("command5")],
            0,
            None,
        )
        ...
        self.assertEqual(
            env.functions["case3_001_ln0"].commands, ["command2", "command5"]
        )
        self.assertEqual(
            env.functions["case4-5_001_ln0"].commands, ["command3", "command4"]
        )
    ...
    def test_mcfunction_switch_cases_five(self):
        ...
        self.assertTrue(
            func.switch_cases(
                "switch_var", [case1, case2, case3, case4, case5]
            )
        )
        self.assertEqual(
            func.commands,
        ...
        self.assertEqual(
            env.functions["switch6-15_001_ln0"].commands,
            [
                "execute if score Global switch_var matches 6..10 run command4",
    ...
    def test_mcfunction_get_vector_path(self):
        ...
        self.assertTrue(
            func.get_vector_path(
                "@test_selector[x=1]",
                "test_var",
                ["assign_var1", "assign_var2", "assign_var3"],
            )
        )
        self.assertEqual(
            func.commands,
    ...
    def test_mcfunction_set_selector_path(self):
        ...
        self.assertTrue(
            func.set_vector_path(
                "@test_selector[x=1]", "test_var", ["x", "y", "z"]
            )
        )
        self.assertEqual(
            func.commands,
