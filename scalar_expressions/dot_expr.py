...
class dot_expr(scalar_expression_base):
    ...
    def compile(self, func, assignto=None):
        ...
        for i in range(3):
            ...
            func.add_command(
                f"scoreboard players operation {prod.selvar} *= {multiplicand.selvar}"
            )

            prods.append(prod)

        func.add_command(
            f"scoreboard players operation {prods[0].selvar} += {prods[1].selvar}"
        )
        func.add_command(
            f"scoreboard players operation {prods[0].selvar} += {prods[2].selvar}"
        )
